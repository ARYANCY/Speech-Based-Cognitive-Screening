...
def predict(raw_text):
    ...
    if max_prob is not None:
        prob_calibrated = apply_temperature_scaling(max_prob)
    else:
...
if __name__ == "__main__":
    if len(sys.argv) >= 2:
        ...
        # Use calibrated probability (prob) which is already the max-class calibrated probability
        confidence = prob

        if confidence is not None and confidence < CONFIDENCE_THRESHOLD:
            prediction_label = "Inconclusive"
            label = None
            flag_reason = f"Confidence {confidence:.1%} below {CONFIDENCE_THRESHOLD:.0%} threshold - please review manually"
        else:
