...
def estimate_speaking_time(text, words_per_minute=150):
    """
    Estimate speaking time based on word count.
    Average speaking rate: 150-160 WPM for normal speech.
    Dementia patients often speak slower (100-120 WPM).
    """
    words = clean_text(text).split()
    num_words = len(words)
    if num_words == 0:
    ...
    estimated_seconds = (num_words / words_per_minute) * 60
    return estimated_seconds

def calculate_hesitation_density(text):
    """
    Calculate density of hesitation markers (fillers, corrections) per word.
    Higher density indicates more pauses and hesitations.
    """
    words = clean_text(text).split()
    if len(words) == 0:
        return 0.0
    ...
    total_hesitations = fillers + corrections
    
    return total_hesitations / len(words)

def calculate_pause_indicators(text):
    """
    Count pause indicators like ellipses, commas, and fillers.
    Dementia patients have more pauses in speech.
    """
    # Count ellipses and multiple punctuation (indicating pauses)
    ellipses = len(re.findall(r'\.{2,}', text))
    # Count commas (often indicate pauses)
    commas = text.count(',')
    # Count fillers
    fillers = count_fillers(text)
    
    return ellipses + commas + fillers
...
def compute_features_for_text(text):
    ...
    hesitation_density = calculate_hesitation_density(text)
    pause_indicators = calculate_pause_indicators(text)
    # Estimated words per minute (based on standard 150 WPM)
    estimated_wpm = (num_words / estimated_time * 60) if estimated_time > 0 else 0.0
    ...
    return {
        ...
        # Time-based features (for display/analysis, not used in model yet)
